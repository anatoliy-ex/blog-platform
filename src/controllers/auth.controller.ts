...
import { AuthUsersService } from '../service/auth.service';
...
export class AuthUsersController {
  constructor(
    protected authUsersService: AuthUsersService,
  ...
  async loginUser() {
    ...
    const isLogin = await this.authUsersService.loginUser(
      req.body,
      userIp,
      deviceId,
      deviceName,
    ...
    if (isLogin) {
      res.cookie('refreshToken', isLogin.refreshToken, {
        httpOnly: true,
        secure: true,
      });
  ...
  async passwordRecovery(req: Request, res: Response) {
    await this.authUsersRepositories.recoveryPasswordWithSendEmail(
      req.body.email,
    );
    res.sendStatus(204);
  }
  ...
  async confirmNewPassword(req: Request, res: Response) {
    const isConfirmation =
      await this.authUsersRepositories.confirmNewPasswordWithCode(
  ...
  async generateRefreshAndAccessToken(req: Request, res: Response) {
    ...
    const oldLastActiveDate =
      await this.jwtTokenService.getLastActiveDateFromToken(oldRefreshToken);
    const sessions = await RefreshTokenSessionModel.findOne({
      deviceId: deviceId,
    });
    ...
    const isGenerate =
      await this.authUsersService.GenerateRefreshAndAccessToken(
        userId,
        deviceId,
        sessions,
        deviceIp,
    ...
    res.cookie('refreshToken', isGenerate.refreshToken, {
      httpOnly: true,
      secure: true,
    });
  ...
  async registrationAndConfirmation(req: Request, res: Response) {
    const confirmationWithCode =
      await this.authUsersRepositories.confirmEmailByUser(req.body.code);
  ...
  async firstRegistration(req: Request, res: Response) {
    const firstRegistration: boolean =
      await this.authUsersRepositories.firstRegistrationInSystem(req.body);
  ...
  async registrationEmailResending(req: Request, res: Response) {
    const isResending =
      await this.authUsersRepositories.registrationWithSendingEmail(
        req.body.email,
      );
